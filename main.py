...
try:
    ...
    # [수정됨] 제목 원본 유지 + 소제목 고정 + 줄바꿈 강제 프롬프트
    gpt_prompt = f"""
    너는 채용 공고나 프로젝트 정보를 정리해주는 '전문 에디터'야.
    아래 [글 내용]을 읽고, 지정된 **출력 양식**을 토씨 하나 틀리지 않고 그대로 지켜서 답변해.

    [필수 지침]
    1. **제목 유지**: '추천 프로젝트' 아래에는 글에 있는 **원래 공고 제목**을 그대로 적어. (절대 요약하거나 바꾸지 마)
    2. **줄바꿈**: 모든 불릿 포인트(•) 사이에는 **반드시 빈 줄(엔터)을 하나씩 넣어.**
    3. **소제목 고정**: 아래 출력 양식에 있는 이모지와 소제목 텍스트를 그대로 써.
