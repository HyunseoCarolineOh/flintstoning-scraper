...
try:
    ...
    for index, row in target_rows.iterrows():
        ...
        try:
			...
			[적합 조건 (TRUE)]
			1. 2와 3의 조건을 동시에 만족해야 합니다. 
			2. 필수 주제 (다음 중 하나라도 직접적인 관련이 있어야 합니다):
               - 콘텐츠 마케팅: 브랜드 전략, 비평 등
               - 글쓰기: 스토리텔링, 에디팅 스킬 등
			   - 브랜드: 브랜드 정체성, 브랜딩 사례, 브랜드 간 협업 등
               - 문화: 문화예술 트렌드, 사회적 현상에 대한 담론, 라이프스타일 분석 
               - 결과물이 ‘텍스트 콘텐츠’ 또는 ‘텍스트 중심 산출물' (예시: 뉴스레터, 아티클, 리포트, 큐레이션, 비평 글 등)
           3. 사이드프로젝트에서 모집하는 포지션에 아래 ‘콘텐츠 관련 직종’이 하나도 포함되어 있지 않으면 FALSE로 판단합니다: 
	            - 마케터
				- 콘텐츠 기획자
				- 에디터
