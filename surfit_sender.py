...
try:
    ...
    for index, row in target_rows.iterrows():
        ...
        try:
            ...
            # 4. ANTIEGG 정체성 판단
            identity_prompt = f"""
            당신은 프리랜서 에디터 공동체 'ANTIEGG'의 편집장입니다. 내용을 읽고 ANTIEGG의 정체성에 부합하는지 판단해 주세요.

            [적합 조건 (TRUE)]
            필수 주제 (다음 중 하나라도 직접적인 관련이 있어야 합니다):
               - 콘텐츠 마케팅: 브랜드 전략, 비평 등
               - 글쓰기: 스토리텔링, 에디팅 스킬, 글 등
            ...
            [사례 학습 (Few-Shot)]
            - ✅ 적합: '네이버와 돌고래유괴단 협업', '제로클릭 시대의 마케팅', '마케터의 커뮤니티 운영 회고', '클로드 코드로 블로그 글 자동화 에이전트 만들기'
            ...
            # [수정] 부적합 시 로그 출력 및 시트 업데이트
            if not is_appropriate:
                print("-" * 60)
                print(f"🚫 [DROP] 부적합 아티클 발견")
                print(f"   제목: {project_title}")
                print(f"   사유: {drop_reason}")
                print("-" * 60)
